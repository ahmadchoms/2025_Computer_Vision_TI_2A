...
def classify_gesture(hand):
    ...
    r_mean = np.mean([dist(index_tip, wrist), dist(middle_tip, wrist), dist(ring_tip, wrist), dist(pinky_tip, wrist)])

    if dist(thumb_tip, index_tip) < 35: return "OK"
    if (thumb_tip[1] < wrist[1]-40) and (dist(thumb_tip, wrist) >0.8 * dist(index_tip, wrist)): return "Thumbs Up"
...
while True:
    ...
    if hands:
        ...
        cv2.putText(img, f"Gesture: {label}", (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 255), 2)
